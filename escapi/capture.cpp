...
// IUnknown methods
STDMETHODIMP ESCCapture::QueryInterface(REFIID aRiid, void** aPpv)
{
	static const QITAB qit[] =
	{
		QITABENT(ESCCapture, IMFSourceReaderCallback),
		{ 0 },
	};
	return QISearch(this, qit, aRiid, aPpv);
}

STDMETHODIMP_(ULONG) ESCCapture::AddRef()
{
	return InterlockedIncrement(&mRefCount);
}

STDMETHODIMP_(ULONG) ESCCapture::Release()
{
...
STDMETHODIMP ESCCapture::OnReadSample(
	HRESULT aStatus,
	DWORD aStreamIndex,
	DWORD aStreamFlags,
	LONGLONG aTimestamp,
...
STDMETHODIMP ESCCapture::OnEvent(DWORD, IMFMediaEvent *)
{
	return S_OK;
}

STDMETHODIMP ESCCapture::OnFlush(DWORD)
{
	return S_OK;
}

int ESCCapture::escapiPropToMFProp(int aProperty)
{
	int prop = 0;
	switch (aProperty)
	{
...
int ESCCapture::setProperty(int aProperty, float aValue, int aAuto)
{
	HRESULT hr;
...
int ESCCapture::getProperty(int aProperty, float &aValue, int &aAuto)
{
	HRESULT hr;
	IAMVideoProcAmp *procAmp = NULL;
	IAMCameraControl *control = NULL;
...
BOOL ESCCapture::isFormatSupported(REFGUID aSubtype) const
{
	int i;
...
HRESULT ESCCapture::getFormat(DWORD aIndex, GUID *aSubtype) const
{
...
HRESULT ESCCapture::setConversionFunction(REFGUID aSubtype)
{
	mConvertFn = NULL;
...
HRESULT ESCCapture::setVideoType(IMFMediaType *aType)
{
	HRESULT hr = S_OK;
	GUID subtype = { 0 };
...
int ESCCapture::isMediaOk(IMFMediaType *aType, int aIndex)
{
	HRESULT hr = S_OK;
...
int ESCCapture::scanMediaTypes(unsigned int aWidth, unsigned int aHeight)
{
	HRESULT hr;
...
HRESULT ESCCapture::initCapture(int aDevice)
{
	mWhoAmI = aDevice;
...
void ESCCapture::getResolution(SimpleRes* aResList, int* aResnum)
{
	HRESULT hr;
	IMFMediaType* nativeType = NULL;
	int count = 0;
	...
	while (nativeTypeErrorCode == S_OK) {
		nativeTypeErrorCode = mReader->GetNativeMediaType((DWORD)MF_SOURCE_READER_FIRST_VIDEO_STREAM, count, &nativeType);
		...
		{
			SimpleRes res;
			hr = MFGetAttributeSize(nativeType, MF_MT_FRAME_SIZE, &res.mWidth, &res.mHeight);
			aResList[index++] = res;
		}
		nativeType->Release();
		nativeType = NULL;
